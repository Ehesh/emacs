...
* Auto-Tangle Configuration
  ...
  (defun tangle-config-org ()
    "Automatically tangle the current Org file if it's 'config.org'."
    (when (string-equal (buffer-file-name)
                        (expand-file-name "~/.emacs.d/config.org")) ;; Set the correct path
      (let ((org-confirm-babel-evaluate nil))  ;; Disable confirmation prompts
        (org-babel-tangle))))
