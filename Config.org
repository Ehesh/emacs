...
- [[#post-init-configuration][Post-Init Configuration]]
  ...
  - [[#projectile-todo-consider-normal-project][PROJECTILE TODO consider normal project]]
  - [[#rainbow-delimiters-todo][RAINBOW DELIMITERS TODO]]
  - [[#rainbow-mode][RAINBOW MODE]]
  - [[#shells-and-terminals][SHELLS AND TERMINALS]]
  - [[#sudo-edit][Sudo EDIT]]
  - [[#tldr][TLDR]]
  - [[#transient][TRANSIENT]]
  - [[#casual-suite-todo-add-the-casual-packages][Casual-Suite TODO add the casual packages]]
  - [[#moody][MOODY]]
...
(use-package ef-themes
  ...
  (setq ef-frost-palette-overrides
        '( 
          (bg-hover "#b3efdf")
       ;;   (bg-hover-secondary "#eab5ff")
          ))
        (setq ef-themes-to-toggle '(ef-owl ef-frost))
    ;; Apply the theme
  (load-theme 'ef-owl t)) 
...
;;     '("SPC" . my-transient-menu)
     '("<escape>" . ignore)
     (cons "m" my-buffer-prefix-keymap)
)
...
** PROJECTILE TODO consider normal project
...
Adding rainbow coloring to parentheses. I need to add this to python programming
...
;; Define the first transient menu
(transient-define-prefix leader-menu ()
  "DT Leader Key Transient Menu"
  ["Main Commands"
   ;; ("SPC" "Counsel M-x" counsel-M-x)
   ("." "Find file" find-file)
   ;;   ("=" "Perspective" perspective-map)
   ("TAB TAB" "Comment lines" comment-line)
   ("u" "Universal argument" universal-argument)])

;; Define the second transient menu for Buffers/Bookmarks
(transient-define-prefix buffers-menu ()
  "Buffers/Bookmarks Menu"
  ["Buffers/Bookmarks"
   ("b" "Switch to buffer" switch-to-buffer)
   ("c" "Clone indirect buffer" clone-indirect-buffer)
   ("C" "Clone indirect in new window" clone-indirect-buffer-other-window)
   ("d" "Delete bookmark" bookmark-delete)
   ("i" "Ibuffer" ibuffer)
   ("k" "Kill current buffer" kill-current-buffer)
   ("K" "Kill multiple buffers" kill-some-buffers)
   ...
   ("w" "Save bookmarks" bookmark-save)])

(transient-define-prefix dired-menu ()
  "Dired Menu"
  ["der-keys"
   ;;   ("f" "Writable dired finish edit"  wdired-finish-edit )
   ("j" "Dired jump to current"  dired-jump )
   ("n" "Open directory in neotree"  neotree-dir )
   ;;   ("p" "Peep-dired"  peep-dired )
   ("w" "Writable dired"  wdired-change-to-wdired-mode)
   ])


(transient-define-prefix e-menu ()
  "Ediff/Eshell/Eval/EWW"
  ["der-keys"
   ("b" "Evaluate elisp in buffer"  eval-buffer )
   ("d" "Evaluate defun containing or after point"  eval-defun )
   ("e" "Evaluate and elisp expression"  eval-expression )
   ("f" "Run ediff on a pair of files"  ediff-files )
   ("F" "Run ediff on three files"  ediff-files3 )
   ("h" which-key "Eshell history"  counsel-esh-history )
   ("l" "Evaluate elisp expression before point"  eval-last-sexp )
   ("r" "Evaluate elisp in region"  eval-region )
   ;; ("R" which-key "Reload current page in EWW"  eww-reload )
   ;; ("s" which-key "Eshell"  eshell )
   ;; ("w" which-key "EWW emacs web wowser"  eww )
   ])


(transient-define-prefix help-menu ()
  "Help"
  ["h-keys"
   ...
   ;; ("d" ignore t   )
   ;; ("d "About Emacs"  a" about-emacs )
   ;; ("d "View Emacs debugging"  d" view-emacs-debugging )
   ;; ("d "View Emacs FAQ"  f" view-emacs-FAQ )
   ;; ("d "The Emacs manual"  m" info-emacs-manual )
   ;; ("d "View Emacs news"  n" view-emacs-news )
   ;; ("d "How to obtain Emacs"  o" describe-distribution )
   ;; ("d "View Emacs problems"  p" view-emacs-problems )
   ;; ("d "View Emacs todo"  t" view-emacs-todo )
   ;; ("d "Describe no warranty"  w" describe-no-warranty )
...
(transient-define-prefix toggle ()
  "Toggle"
  ["toggls"
   ("n" "Toggle neotree file viewer"  neotree-toggle )
   ("l" "Toggle line numbers"  display-line-numbers-mode )
   ("t" "Toggle truncated lines"  visual-line-mode )
   ("o" "Toggle org mode"  org-mode )
   ("r" "Toggle rainbow mode"  rainbow-mode )
   ;; ("v" "Toggle vterm"  vterm-toggle )
   ;; ("e" "Toggle eshell"  eshell-toggle )
   ;; ("f" "Toggle flycheck"  flycheck-mode )
   ])
...
;; Bind the transient menus to keys

#+end_src

** Casual-Suite TODO add the casual packages
...
;;   :ensure (casual-editkit :host github :repo "kickingvegas/casual-isearch")
...
(defun my-update-modeline-color-based-on-meow-state ()
  ...
  (let ((insert-state-color (ef-themes-get-color-value 'bg-hover-secondary))  ;; Green color for insert mode
        ;; Use modus theme aliases for the modeline in normal state
        (normal-state-color (ef-themes-get-color-value 'bg-mode-line)))
